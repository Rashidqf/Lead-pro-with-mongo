...
//#region node_modules/.nitro/vite/services/ssr/assets/auth.functions-xdkSkTST.js
...
var getMe = createServerFn({ method: "GET" }).handler(getMe_createServerFn_handler, async () => {
	const { readSessionToken, verifyToken } = await import("./session.server-DKz0L2Oo.mjs").then((n) => n.t).then((n) => n.t);
...
var signIn = createServerFn({ method: "POST" }).inputValidator((input) => credentials.pick({
	...
	const { getDb, col } = await import("./client.server-COnOrad6.mjs").then((n) => n.t).then((n) => n.t);
	const { signToken, setSessionCookie } = await import("./session.server-DKz0L2Oo.mjs").then((n) => n.t).then((n) => n.t);
	const db = await getDb();
...
var signOut = createServerFn({ method: "POST" }).handler(signOut_createServerFn_handler, async () => {
	const { setSessionCookie } = await import("./session.server-DKz0L2Oo.mjs").then((n) => n.t).then((n) => n.t);
